...
const calculateSimultaneousSwipes = (code, latestSwipeEvent) => {
    ...
    swipeEvents = swipeEvents.filter(({ timestamp }) => Math.abs(timestamp - latestSwipeEvent.timestamp) <= timeThreshold);
    ...
    if (roomSwipeEvents.length > 0) {
        for (let i = roomSwipeEvents.length - 1; i >= 0; i--) {
            ...
            if (clientA && clientB) {
                ...
                const input = { clientA: primaryClient, swipeA, clientB: secondaryClient, swipeB };
